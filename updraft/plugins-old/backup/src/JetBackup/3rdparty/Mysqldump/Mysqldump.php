...
class Mysqldump
{
	...
	public function __construct(
		...
		/**
		 * Only set unbuffered query mode if using MySQL and the constant is available.
		 *
		 * - We cannot define PDO::MYSQL_ATTR_USE_BUFFERED_QUERY manually because it's a class constant;
		 *   PHP will throw a fatal error if we try to access it without the 'pdo_mysql' extension loaded.
		 * - If the constant is not defined, attempting to use it will result in a fatal error.
		 * - This constant controls whether MySQL uses buffered queries. If omitted, queries will still work,
		 *   but large result sets may consume more memory since the entire result is buffered in RAM.
		 * - This issue hasn't occurred before because our code only recently started explicitly using
		 *   MYSQL_ATTR_USE_BUFFERED_QUERY - until now, it relied on the default behavior of PDO,
		 *   and we’ve only worked with MySQL where the extension was assumed to be loaded.
		 */
		if ("mysql" === $this->dbType && defined('PDO::MYSQL_ATTR_USE_BUFFERED_QUERY')) {
		...
		if (!$this->dumpSettings['no-create-info']) {
			...
			foreach ($this->dbHandler->query($stmt) as $r) {
				...
				if ($this->dumpSettings['add-drop-table'] &&
					!$this->matches($tableName, $this->dumpSettings['no-data'])) {
					$this->compressManager->write(
